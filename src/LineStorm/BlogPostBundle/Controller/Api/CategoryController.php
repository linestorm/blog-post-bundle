...
{
    private $formName = 'linestorm_blog_form_post_category';
    ...
    public function newAction()
    {
        ...
        $form = $this->createForm($this->formName, null, array(
            'action' => $this->generateUrl('linestorm_blog_post_module_api_post_category'),
            'method' => 'POST',
        ));
    ...
    public function postAction()
    {
        ...
        if ($form->isValid()) {
            ...
            $view = View::create($category, 201, array(
                'location' => $this->generateUrl('linestorm_blog_post_module_api_get_category', array( 'id' => $category->getId() ))
            ));
        } else {
    ...
    public function putAction($id)
    {
        ...
        if ($form->isValid())
        {
            ...
            $view = $this->createResponse(array('location' => $this->generateUrl('linestorm_blog_post_module_api_get_category', array( 'id' => $updatedCategory->getId()))), 200);
        }
        else
        {
            $view = View::create($form);
